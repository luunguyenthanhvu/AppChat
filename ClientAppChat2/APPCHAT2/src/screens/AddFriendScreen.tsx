...
import { View, Text, TouchableOpacity, TextInput, FlatList, StyleSheet, ActivityIndicator, Image } from 'react-native';
import axios from 'axios';
import Icon from 'react-native-vector-icons/FontAwesome';
import { BACKEND_URL_HTTP } from "../config/config";
import AsyncStorage from '@react-native-async-storage/async-storage';

// Định nghĩa kiểu dữ liệu cho bạn bè, yêu cầu kết bạn, liên hệ và nhóm
interface Friend {
    userId: any;
    email: string;
    username: string;
...
const AddFriendScreen = ({ navigation }: any) => {
    ...
    const fetchPotentialFriends = async () => {
        ...
        try {
            ...
            const updatedFriends = response.data.map((friend: { friendStatus: string; }) => ({
                ...friend,
    ...
    // Thay đổi tab và gọi fetch dữ liệu tương ứng
    ...
    // Hủy yêu cầu kết bạn
    const handleCancelRequest = async (friendEmail: string) => {
        if (!email) return;
        try {
    ...
    // Xử lý tìm kiếm bạn bè, yêu cầu kết bạn, liên hệ hoặc nhóm
